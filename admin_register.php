...
if (isset($_SESSION['admin_id'])) {
    ...
    header("Location: admin_dashboard.php"); // Corrected redirect path
    exit();
}
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($stmt->num_rows > 0) {
        ...
        if (password_verify($pass, $hashed_password)) {
            ...
            $_SESSION['admin_id'] = $id;  // Changed session variable name
            $_SESSION['username'] = $user;
            $_SESSION['loggedin'] = true;
            $_SESSION['role'] = $role; // Set the role

            header("Location: admin_dashboard.php"); // Corrected redirect path.  IMPORTANT
            exit();
        } else {
